    ...
    @objc func signInWithGoogle() {
        ...
        GIDSignIn.sharedInstance.signIn(withPresenting: self) { signInResult, error in
            ...
            DatabaseManager.shared.userExists(with: email) { exists in
                if !exists {
                    //insert to database
                    let chatUser = ChatAppUser(firstName: firstName, lastName: lastName, countryCode: 0, phoneNumber: 0000, password: "", emailAddress: email, isOnline: isOnline, lastOnline: lastOnl, friends: [], requests: [])
